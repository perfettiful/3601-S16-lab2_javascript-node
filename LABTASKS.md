...
> The purpose of app.js is blah. When accessing "kittens" users are blah, with "petForm" users are blah, and submitting a form through "petForm" does blah. A route is a set of instructions for how we want the server to satisfy a certain get request for all the different part or pages of our website. For example, actually send them a page, send them a message, or an error message.
